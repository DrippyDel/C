...
def result():
    ...
    f1 = open(cwd + '/samplesolutionsp1.txt', "r")
    f2 = open(cwd + '/smallprogram1solution_student_output.txt', "r")
...
def compilerunfile():
    ...
    for subdir, dirs, files in os.walk(cwd):
        for myfile in files:
            if myfile[:13] == 'smallprogram1' and myfile[-2:] == '.c':
                gcccmd = "gcc -o " + "smallprogram1 " + myfile + " -lm" #compile solution file command and generate executable called smallprogram1
    ...
    f = open('smallprogram1solution_student_output.txt', "w")
    ...
    try:
        proc2 = subprocess.run([crun], capture_output=True, input = inputs, text = True, shell = True, timeout= 0.003)
    except subprocess.TimeoutExpired as e:
        if(e.stdout == None):
            print("Your code did not run successfully and did not capture output.")
            return
...
def setupchecker():
    ...
    if not os.path.exists(cwd + "/samplesolutionsp1.txt"):
    ...
    if flag == 0:
        raise FileNotFoundError("You are missing your C source!! Please put in your smallprogram1_lastname_firstname.c file. Script Exiting!")
