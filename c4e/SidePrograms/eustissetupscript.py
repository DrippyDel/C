...
def result():
    ...
    f1 = open(cwd + '/samplesolutionsp0.txt', "r")
    f2 = open(cwd + '/smallprogram0solution_student_output.txt', "r")
...
def compilerunfile():
    ...
    for subdir, dirs, files in os.walk(cwd):
        for myfile in files:
            if myfile[:13] == 'smallprogram0' and myfile[-2:] == '.c':
                gcccmd = "gcc -o " + "smallprogram0 " + myfile + " -lm" #compile solution file command and generate executable called smallprogram0
                ...
                if(len(str(proc.stderr)) > 0):
                    ...
                    print("------------------------------------------------------------------------")
        break #prevent recursion stay in directory
    
    crun = "./" + "smallprogram0"
    
    f = open('smallprogram0solution_student_output.txt', "w")
...
def setupchecker():
    ...
    if not os.path.exists(cwd + "/samplesolutionsp0.txt"):
    ...
    if flag == 0:
        raise FileNotFoundError("You are missing your C source!! Please put in your smallprogram0_lastname_firstname.c file. Script Exiting!")
