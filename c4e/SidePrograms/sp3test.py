...
def result():
    ...
    f1 = open(cwd + '/samplesolutionsp3.txt', "r")
    f2 = open(cwd + '/smallprogram3solution_student_output.txt', "r")
...
def compilerunfile():
    ...
    for subdir, dirs, files in os.walk(cwd):
        for myfile in files:
            if myfile[:13] == 'smallprogram3' and myfile[-2:] == '.c':
                gcccmd = "gcc -o " + "smallprogram3 " + myfile + " -lm" #compile solution file command and generate executable called smallprogram1
    ...
    f = open('smallprogram3solution_student_output.txt', "w")
...
def setupchecker():
    ...
    if not os.path.exists(cwd + "/samplesolutionsp3.txt"):
    ...
    if flag == 0:
        raise FileNotFoundError("You are missing your C source!! Please put in your smallprogram3_lastname_firstname.c file. Script Exiting!")
